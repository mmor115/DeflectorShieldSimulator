This guide shows how to catch a NaN or a non-normalised state during a run, and how to choose what the simulator does when it finds one.
...
The normalization validator checks whether a state is normalised, within a tolerance:

- For a massive particle, a normalised state satisfies `e = 1 / sqrt(1 - v²)`, where `v²` is the squared speed.
- For a photon, a normalised state satisfies `|v| = 1`.

See [Physics, units and time](units.html) for what a normalised state means physically.

## The four behaviours

Set a separate behaviour for the NaN validator and the normalization validator, independently:
...
**For debugging a specific blow-up**, set the relevant validator to **Die**. The snapshot taken just before the panic captures the exact bad values. Load the dump afterward to inspect the state that triggered the failure. See [Export trajectory data](exporting.html) and [Analyse dumps with warpsim.py](analysis.html).
