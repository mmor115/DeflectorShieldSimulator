...
There is no installer and no separate asset folder. The ship sprite is compiled into the
executable, so you can move the file anywhere.
...
You should see a dark window with a small ship at the centre, two blue rings around it,
and coloured particles entering from the right. The **Parameters** window sits at the top
right.
