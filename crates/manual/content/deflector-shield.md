...
All the field functions below are evaluated in coordinates centred on the bubble, so
$x$ means $x - x_{\mathrm{b}}(t)$, and

$$r = \sqrt{x^2 + y^2 + z^2 + \epsilon}$$

The $\epsilon$ is a guard against dividing by zero at the centre, not a physical quantity.
...
bump centred on the sphere $r = r_0$ with a reach of $s_0$ to either side. In words:
...
drifting relative to the bubble centre, and the drift accumulates.
...
> [!NOTE]
> The paper analyses the metric with a piecewise-linear transition function, because it
...
What the simulator shows is the kinematics of that idea. It does not evaluate whether the
stress-energy required to hold this geometry is physically available. See
[Limits of the model](limits.html).
