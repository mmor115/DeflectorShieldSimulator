...
| Field | Type | Meaning |
|---|---|---|
| `nan_validator` | string | The behaviour for a NaN in a state vector. One of `"Ignore"`, `"Die"`, `"RemoveParticle"`, `"ExplodeParticle"`. |
| `normalization_validator` | string | The behaviour for a state vector that is not normalised within tolerance. Same four values. |
...
The whole `validator_settings` section may be omitted. If it is missing, the simulator applies its defaults: `nan_validator: "Die"`, `normalization_validator: "ExplodeParticle"`, `normalized_tolerance: 1e-6`, `normalized_tolerance_power: -6`. See [Validate numerics](validation.html) for what each behaviour does.
...
> [!NOTE]
> `e` is the seventh component of the state vector. For a massive particle it is the Lorentz factor; for a photon it carries no independent value beyond what normalisation requires. See [Physics, units and time](units.html) for the full definition of a normalised state.
