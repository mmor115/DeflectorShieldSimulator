...
dark window with a ship at the centre, two blue rings around it, and coloured particles
...
Scroll the wheel forward. The view zooms in, and the ship stays centred because the
camera follows it.
...
size does not change, so the trajectory is exactly the one you would have got at full
...
| Read a dump in Python | [Analyse dumps with warpsim.py](analysis.html) |
| Know what a run does not prove | [Limits of the model](limits.html) |
