...
zero-expansion drive. It is a comparison case for the deflector shield, not the subject
of the paper.
...
The Natário drive is spherically symmetric about the bubble centre. It uses the radius
...
**Bubble** tab are therefore greyed out while it is selected:
...
Hovering a greyed-out slider shows "Selected warp drive does not use this parameter."
...
of the "Shield Drag out of range!" behaviour described in
[Parameters](parameters.html) applies. **Speed** moves freely over its whole range.

## The ship

Started fresh, the ship sits at the bubble centre with zero coordinate velocity and is
carried by the shift vector alone. This is the usual warp-drive picture: the ship is at
rest locally, and the geometry does the travelling.
...
This drive is here so you can watch particle geodesics in a bubble that has *no*
deflector, and compare them against the same configuration with the deflector switched
on. That comparison is the point.

It is not a reproduction of Natário's published solution, it has not been checked
against the energy conditions, and the paper makes no claims about it. Everything in
[Limits of the model](limits.html) applies here at least as strongly.
