...
5. **Pre-snapshot validators** — check for NaN and for lost normalisation.
...
so each trajectory depends only on the shared parameters and its own state. See
...
| Particle colour | The $z$ coordinate, mapped across the range $-15$ to $15$ in physics units. |
...
The ship sprite is compiled into the executable, so there is no asset directory to ship
and nothing to lose when the binary is moved. Everything else drawn on screen is generated
at runtime from primitive meshes.
