...
physics, but on screen it only changes the colour and the draw order, so a particle with a
...
Photons obey a different normalisation: their speed is fixed at $c$, so the velocity
