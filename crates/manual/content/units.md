...
Nothing in the program is expressed in metres or seconds. The metric has no intrinsic
...
| $e$ | The energy factor that keeps the state normalised |
...
A state is valid only if it satisfies the right normalisation for its particle type.
...
So a physics length of `4` draws as `40` game units. A slider labelled in game units is
ten times the physics quantity.

> [!NOTE]
> The $z$ coordinate is deliberately **not** scaled. In a two-dimensional view $z$ only
> orders the draw and picks the colour, and multiplying it by ten made particles vanish
